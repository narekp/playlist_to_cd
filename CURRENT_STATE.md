...
- **Local macOS packaging** via `packaging/build_macos.sh`: builds `playlist_to_cd.app` and a local distribution DMG (`playlist_to_cd_0.1.0.dmg`) using `hdiutil` (unsigned, local distribution only).
...
- **Signing/notarization** — deferred; current builds are unsigned and local-only.
